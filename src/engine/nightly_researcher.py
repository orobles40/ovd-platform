...
async def synthesize(
    queries: list[str], results: list[SearchResult], stack_desc: str
) -> str:
...
async def publish_cve_alert(
    org_id: str, project_name: str, synthesis: str, cve_ids: list[str]
) -> None:
...
async def run_nightly_research() -> dict:
    ...
    log.info(
        "nightly_researcher: iniciando job — %s UTC",
    ...
    for org in orgs:
        ...
        for project in projects:
            ...
            # Indexar en RAG
            title = (
                f"Nightly research — {stack_desc} — {started_at.strftime('%Y-%m-%d')}"
            )
            metadata = {
...
def start_scheduler() -> None:
    ...
    if not _NIGHTLY_ENABLED:
        log.info(
            "nightly_researcher: scheduler desactivado (OVD_NIGHTLY_ENABLED=false)"
        )
        return
