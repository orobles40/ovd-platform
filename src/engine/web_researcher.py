...
class WebResearcher:
    ...
    async def research(
        ...
        if curated_urls:
            ...
            log.info(
                "web_researcher: %d fuentes curadas incorporadas", len(curated_results)
            )

        # Paso 1b: buscar en web para cada query
        for query in queries:
            results = await self._provider.search(
                query, max_results=_MAX_RESULTS_PER_QUERY
            )
        ...
        if not all_results:
            return ResearchFindings(
                queries=queries,
                results=[],
                synthesis="Sin resultados de búsqueda.",
                indexed=0,
            )
    ...
    async def research_uncertainties(
        ...
        high_prio = [
            ...
            if u.get("severity") in ("high", "critical", "medium")
        ]
        if not high_prio:
            return ResearchFindings(queries=[], results=[], synthesis="", indexed=0)
        ...
        log.info(
            "web_researcher (uncertainties): %d queries desde %d incertidumbres",
            len(queries),
            len(uncertainties),
        )
        return await self.research(
            queries=queries, context=context, curated_urls=curated_urls
        )
            ...
            for url in urls[:10]:  # máx 10 fuentes curadas
                try:
                    resp = await client.get(
                        url, headers={"User-Agent": "OVD-WebResearcher/1.0"}
                    )
                    if resp.status_code == 200:
                        ...
                        snippet = clean[:800]
                        results.append(
                            SearchResult(title=url, url=url, snippet=snippet)
                        )
                        log.debug(
                            "web_researcher curated: %s → %d chars",
                            url[:60],
                            len(snippet),
                        )
                    else:
                        log.debug(
                            "web_researcher curated: %s → HTTP %d (ignorado)",
                            url[:60],
                            resp.status_code,
                        )
                except Exception as e:
                    log.debug(
                        "web_researcher curated: %s → error %s (ignorado)", url[:60], e
                    )
        return results
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                ...
                if resp.status_code in (200, 201):
                    log.info(
                        "web_researcher: síntesis indexada en RAG org-level (%d chars)",
                        len(synthesis),
                    )
                    return 1
                else:
                    log.warning(
                        "web_researcher: RAG index falló con status %d",
                        resp.status_code,
                    )
        except Exception as e:
...
async def load_curated_urls(
    org_id: str, project_id: str | None, db_url: str
) -> list[str]:
    """
