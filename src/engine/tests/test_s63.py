...
def test_s63d_template_rut_9999999_dv_is_3():
    ...
    tpl = pathlib.Path(__file__).parent.parent / "templates" / "stack" / "backend_python.md"
    ...
    assert "9.999.999-3" in content, \
        "template no documenta que DV de 9.999.999 es 3"
...
def test_s63d_template_rut_invalid_case_documented():
    ...
    assert "9.999.999-K" in content, "template debería mencionar 9.999.999-K como caso inválido"
...
def test_s63d_validate_rut_reference_implementation():
    ...
    assert validate_rut("9.999.999-K") is False,  "9.999.999-K debe ser INVÁLIDO (DV real=3)"
...
def test_s63a_status_reducer_is_callable():
    ...
    assert result == "new_value", "el reducer debe retornar el segundo valor (last-write-wins)"
...
def test_s63b_cleanup_uses_agent_results_not_rglob(tmp_path):
    ...
    state = {
        "test_results": {"passed": False, "output": "AssertionError: assert False == True\n", "return_code": 1},
        "retry_feedback": "",
        "last_test_error": "",
        "test_retry_count": 1,
    ...
    assert (tmp_path / "pytest.ini").exists(), "pytest.ini NO debe ser borrado por S63-B"
...
def test_s63b_cleanup_not_in_run_tests(tmp_path):
    ...
    assert service.exists(), "S63-B: run_tests NO debe borrar src/main.py (es del retry actual)"
