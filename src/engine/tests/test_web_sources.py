...
async def test_research_no_curated_skips_fetch():
    ...
    fake_results = [
        SearchResult(title="T", url="https://example.com", snippet="snippet")
    ]
    ...
    with (
        patch.object(
            researcher, "_fetch_curated", new_callable=AsyncMock
        ) as fetch_mock,
        patch.object(
            researcher, "_synthesize", new_callable=AsyncMock, return_value="sintesis"
        ),
        patch.object(
            researcher, "_index_to_rag", new_callable=AsyncMock, return_value=1
        ),
    ):
...
async def test_research_with_curated_calls_fetch_and_prepends():
    ...
    curated_result = SearchResult(
        title="https://docs.oracle.com",
        url="https://docs.oracle.com",
        snippet="oracle docs",
    )
    search_result = SearchResult(
        title="Search", url="https://search.com", snippet="search result"
    )
...
async def test_fetch_curated_returns_search_result():
    ...
    mock_resp.text = (
        "<html><body><p>Oracle 19c documentation content here</p></body></html>"
    )
    ...
    with patch("web_researcher.httpx.AsyncClient", return_value=mock_client):
        results = await researcher._fetch_curated(
...
async def test_run_web_research_passes_curated_urls():
    from web_researcher import ResearchFindings, run_web_research

    fake_findings = ResearchFindings(
        queries=["q"], results=[], synthesis="ok", indexed=0
    )

    with patch(
        "web_researcher.WebResearcher.research",
        new_callable=AsyncMock,
...
async def test_load_curated_urls_returns_empty_on_db_error():
    ...
    with patch.object(
        psycopg.AsyncConnection, "connect", side_effect=Exception("db down")
    ):
