...
class TestS31AMtimeFilter:
    ...
    def test_mtime_filter_logica(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "old_migration.sql" not in found_files, (
                "Archivo viejo debe excluirse"
            )
...
class TestS31BRetryFeedbackCap:
    ...
    async def test_update_test_retry_trunca_feedback(self):
        ...
        result = (
            await _graph.update_test_retry(state)
            if asyncio_needed()
            else _graph.update_test_retry(state)
        )
    ...
    def test_update_qa_retry_trunca_feedback(self):
        ...
        assert "S31-B" in src or "_truncate" in src, (
            "Cap S31-B no encontrado en update_qa_retry"
        )
    ...
    def test_update_test_retry_tiene_cap(self):
        ...
        assert "S31-B" in src or "_truncate" in src, (
            "Cap S31-B no encontrado en update_test_retry"
        )
