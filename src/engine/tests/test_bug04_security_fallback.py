...
from graph import SecurityAuditOutput, _parse_security_fallback
...
class TestParseFallbackJSON:
    ...
    def test_json_score_negativo(self):
        ...
        raw = (
            '{"score": -10, "passed": false, "severity": "critical", "summary": "bad"}'
        )
        result = _parse_security_fallback(raw)
        ...
        raw = json.dumps(
            {
                ...
                "secrets_found": [],
                "insecure_patterns": ["SQL concatenation sin parametrizar"],
                "rls_compliant": False,
                "remediation": ["Usar queries parametrizadas"],
        ...
        raw = json.dumps(
            {
                ...
                "severity": "critical",
                "vulnerabilities": [],
                "secrets_found": ["AWS_SECRET_KEY hardcoded"],
                "insecure_patterns": [],
                "rls_compliant": True,
                "remediation": [],
    ...
    def test_severidades_validas(self):
        ...
        for texto in ["", "alto riesgo", "crítico", "low severity", "MEDIUM issue"]:
            ...
            assert result.severity in valid, (
                f"severity inválido '{result.severity}' para: {texto}"
            )
