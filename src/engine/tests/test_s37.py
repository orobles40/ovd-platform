...
class TestS37BDeliveryReportPath:
    ...
    def test_ruta_absoluta_en_tmpdir(self, tmp_path):
        ...
        state = {
            ...
            "agent_results": [],
            "security_result": {"score": 100, "passed": True, "severity": "none",
                                "vulnerabilities": [], "secrets_found": [], "insecure_patterns": [],
                                "rls_compliant": True, "remediation": [], "summary": "ok"},
            "qa_result": {"score": 90, "passed": True, "issues": [], "sdd_compliance": True,
                         "missing_requirements": [], "code_quality_issues": [], "summary": "ok"},
        ...
        result_path = g._generate_delivery_report(state, [], 0.0, "1m 0s", 0, 0, "ollama")
        if result_path:
            p = pathlib.Path(result_path)
            assert p.is_absolute(), \
                f"S37-B: ruta retornada '{result_path}' no es absoluta"
            assert p.exists(), \
                f"S37-B: archivo '{result_path}' no existe en disco"
