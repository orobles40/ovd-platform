...
class TestS73E:
    def test_template_includes_dots_format_as_valid(self):
        ...
        assert "validate_rut('12.345.678-5') == True" in content or \
    ...
    def test_template_does_not_mark_dots_format_as_invalid(self):
        ...
        assert "validate_rut('12.345.678" not in content or \
               "== False" not in content.split("validate_rut('12.345.678")[1][:30] \
               if "validate_rut('12.345.678" in content else True

    def test_template_includes_without_dots_format(self):
        """S73-E: template también incluye formato sin puntos."""
        content = _BACKEND_PY.read_text(encoding="utf-8")
        assert "validate_rut('12345678" in content or 'validate_rut("12345678' in content
