...
class TestS48ASecurityAuditBypass:
    ...
    def test_security_audit_has_bypass_check(self):
        ...
        assert "OVD_SECURITY_MIN_SCORE" in content, "Falta chequeo de OVD_SECURITY_MIN_SCORE en security_audit"
    ...
    async def test_security_audit_returns_bypass_when_score_zero(self):
        ...
        with patch.dict(os.environ, {"OVD_SECURITY_MIN_SCORE": "0"}):
            with patch.object(g.model_router, "get_llm_with_context", new_callable=AsyncMock) as mock_llm:
    ...
    async def test_security_audit_runs_llm_when_score_nonzero(self):
        ...
        with patch.dict(os.environ, {"OVD_SECURITY_MIN_SCORE": "70", "OVD_SECURITY_SCAN_ENABLED": "false"}):
            with patch.object(g.model_router, "get_llm_with_context", new_callable=AsyncMock) as mock_router:
...
class TestS48BSddInterfaceContract:
    ...
    def test_sdd_interface_contract_prevents_name_mismatch(self):
        ...
        content = _sdd_template()
        assert "ImportError" in content or "mismo nombre" in content.lower() or "mismas clases" in content.lower(), \
