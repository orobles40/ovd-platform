...
class TestS30AWriteFileDirname:
    ...
    def test_write_file_subdirectorio_crea_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = write_fn.invoke({"path": "src/components/Button.tsx", "content": "export default () => <button/>"})
            assert not result.startswith("ERROR"), f"No debería fallar: {result}"
            assert os.path.isfile(os.path.join(tmpdir, "src", "components", "Button.tsx"))
    ...
    def test_write_file_retorna_ruta_absoluta(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert os.path.isfile(result), f"Archivo no existe en la ruta retornada: {result}"
...
class TestS30BLogging:
    ...
    def test_no_written_files_emite_warning(self):
        ...
        import graph as _graph
        src = inspect.getsource(_graph._run_agent_with_tools)
        assert "no escribió ningún archivo" in src or "written_files=0" in src.lower() or \
               "S30-B" in src
...
class TestS30CSubdirectoryInstruction:
    ...
    def test_human_content_prohibe_rutas_planas(self):
        ...
        import graph as _graph
        src = inspect.getsource(_graph._run_agent_with_tools)
        assert "NO uses rutas planas" in src or "no uses rutas planas" in src.lower() or \
               "write_file" in src, \
...
class TestS30DMessageCompression:
    ...
    def test_message_compression_code_exists(self):
        ...
        assert "_MAX_HIST" in src or "MAX_HIST" in src, "Variable _MAX_HIST no encontrada"
    ...
    def test_compression_mantiene_system_y_human(self):
        ...
        assert "messages[:2]" in src, "No se conservan los primeros 2 mensajes en la compresión"
    ...
    async def test_context_no_explota_con_muchas_iteraciones(self):
        ...
        for i in range(20):
            ...
            messages.append(ToolMessage(content=f"tool_result {i}", tool_call_id=f"tc{i}"))
        ...
        assert len(compressed) <= _MAX_HIST + 2, f"Mensajes no comprimidos: {len(compressed)}"
        assert isinstance(compressed[0], SystemMessage), "Primer mensaje debe ser SystemMessage"
        assert isinstance(compressed[1], HumanMessage), "Segundo mensaje debe ser HumanMessage"
