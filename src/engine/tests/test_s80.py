...
class TestS80A:
    ...
    def test_manifest_vacio_con_orm_imports_es_error(self, tmp_path):
        ...
        self._write(tmp_path, "src/contracts/models.py",
            "from pydantic import BaseModel\n"
            "class ContractBase(BaseModel): pass\n"
            "class ContractCreate(BaseModel): pass\n"
        )
        self._write(tmp_path, "src/contracts/service.py",
    ...
    def test_manifest_vacio_multiples_orm_imports(self, tmp_path):
        ...
        self._write(tmp_path, "src/benefits/models.py",
            "from pydantic import BaseModel\n"
            "class BenefitCreate(BaseModel): pass\n"
        )
        self._write(tmp_path, "src/benefits/service.py",
...
class TestS80D:
        ...
        return {
            "tasks": [
                {"id": "T1", "agent": "backend", "file": "src/auth/router.py",
                 "title": "Auth router", "description": "Login endpoint"},
                {"id": "T2", "agent": "backend", "file": "src/contracts/router.py",
                 "title": "Contracts router", "description": "CRUD contratos"},
            ]
        }
    ...
    def test_no_inyecta_auth_router_si_sdd_no_tiene_auth_router_py(self):
        ...
        from graph import _ensure_fastapi_main_task
        sdd = {"tasks": [
            {"id": "T1", "agent": "backend", "file": "src/contracts/router.py",
             "title": "Contracts", "description": "CRUD"},
        ]}
        result = _ensure_fastapi_main_task(sdd, {"raw": "api rest con jwt login endpoint", "type": "backend"})
        main_task = next((t for t in result["tasks"] if t.get("file") == "src/main.py"), None)
        assert main_task is not None
    ...
    def test_no_duplica_auth_router_si_ya_esta(self):
        ...
        result = _ensure_fastapi_main_task(sdd, {"raw": "api rest login jwt auth endpoint", "type": "backend"})
        main_task = next((t for t in result["tasks"] if t.get("file") == "src/main.py"), None)
        assert main_task is not None
    ...
    def test_no_inyecta_auth_router_sin_auth_en_fr(self):
        ...
        from graph import _ensure_fastapi_main_task
        sdd = {"tasks": [
            {"id": "T1", "agent": "backend", "file": "src/imc/router.py",
             "title": "IMC", "description": "calcular IMC"},
        ]}
        result = _ensure_fastapi_main_task(sdd, {"raw": "calculadora de imc", "type": "backend"})
        main_task = next((t for t in result["tasks"] if t.get("file") == "src/main.py"), None)
        if main_task:
            assert "auth_router" not in main_task["description"]
