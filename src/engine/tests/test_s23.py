...
class TestDeliverUsesExistingArtifacts:
    ...
    @pytest.mark.asyncio
    async def test_deliver_falls_back_to_write_artifacts_when_no_directory(
        self, tmp_path
    ):
            ...
            agent_results=[
                {
                    ...
                    "artifacts": [
                        {"path": "src/utils.py", "size": 30, "lang": "python"}
                    ],
                }
            ],
        )
    ...
    @pytest.mark.asyncio
    async def test_deliver_calls_write_artifacts_when_no_existing_artifacts(
        self, tmp_path
    ):
...
class TestQaReviewReadsFromDisk:
    ...
    async def test_qa_reads_files_from_disk_when_artifacts_present(self, tmp_path):
            ...
            agent_results=[
                {
                    ...
                    "output": "",  # vacío — tool calling path
                    "artifacts": [
                        {"path": "src/utils.py", "size": 50, "lang": "python"}
                    ],
                }
            ],
            sdd={
                "summary": "Función promedio",
                "requirements": [],
                "design": {},
                "constraints": [],
                "tasks": [],
        ...
        async def mock_invoke(messages):
            ...
            return MagicMock(
                content='{"passed": true, "score": 80, "issues": [], "sdd_compliance": true, "missing_requirements": [], "code_quality_issues": [], "summary": "OK"}'
            )
        ...
        with (
            ...
            mock_structured.side_effect = AsyncMock(
    ...
    async def test_qa_falls_back_to_output_when_no_directory(self):
        ...
        async def mock_invoke(messages):
            captured_prompt.extend(str(m.content) for m in messages)
            return MagicMock(
                content='{"passed": true, "score": 75, "issues": [], "sdd_compliance": true, "missing_requirements": [], "code_quality_issues": [], "summary": "OK"}'
            )
