...
_WORKSPACE_RS = (_DESKTOP_DIR / "src-tauri" / "src" / "workspace.rs").read_text(encoding="utf-8")
_LIB_RS       = (_DESKTOP_DIR / "src-tauri" / "src" / "lib.rs").read_text(encoding="utf-8")
_TAURI_TS     = (_DESKTOP_DIR / "frontend" / "src" / "lib" / "tauri.ts").read_text(encoding="utf-8")
_SETUP_TS     = (_DESKTOP_DIR / "frontend" / "vitest.setup.ts").read_text(encoding="utf-8")
_WORKSPACE_TSX = (_DESKTOP_DIR / "frontend" / "src" / "pages" / "Workspace.tsx").read_text(encoding="utf-8")
_FRLAUNCHER_TSX = (_DESKTOP_DIR / "frontend" / "src" / "pages" / "FrLauncher.tsx").read_text(encoding="utf-8")
...
def test_s127c_checkout_uses_create_flag():
    assert '"-b"' in _WORKSPACE_RS or '"-b", &branch' in _WORKSPACE_RS or "create" in _WORKSPACE_RS
...
def test_s127l_checkout_uses_create_true():
    assert "workspaceGitCheckoutBranch(project.directory, newBranchName, true)" in _FRLAUNCHER_TSX
