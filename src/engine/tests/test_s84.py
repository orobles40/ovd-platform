...
def test_s84b_content_not_empty(tmp_path):
    ...
    agent_output = '```python:src/contracts/models.py\nclass ContractORM:\n    id = 1\n```'
    result = _write_artifacts(agent_output, str(tmp_path), agent="backend")
...
def test_s84b_content_enables_dependency_context(tmp_path):
    ...
    from graph import _write_artifacts, _build_dependency_context

    agent_output = '```python:src/contracts/models.py\nclass ContractORM:\n    pass\n```'
    written = _write_artifacts(agent_output, str(tmp_path), agent="backend")
    written_context = {art["path"]: art["content"] for art in written if art.get("content")}
...
def test_s84f_injects_when_router_present_no_models():
    ...
    sdd = {"tasks": [
        {"id": "T1", "file": "src/auth/router.py", "agent": "backend", "title": "auth router"},
    ]}
...
def test_s84f_no_inject_when_models_exists():
    ...
    sdd = {"tasks": [
        {"id": "T1", "file": "src/auth/models.py", "agent": "backend", "title": "auth models"},
        {"id": "T2", "file": "src/auth/router.py", "agent": "backend", "title": "auth router"},
    ]}
    result = _ensure_auth_models_task(sdd)
    auth_model_tasks = [t for t in result["tasks"] if "auth/models" in t.get("file", "")]
...
def test_s84f_no_inject_when_no_auth_router():
    ...
    sdd = {"tasks": [
        {"id": "T1", "file": "src/contracts/models.py", "agent": "backend", "title": "contracts"},
    ]}
    result = _ensure_auth_models_task(sdd)
    auth_model_tasks = [t for t in result["tasks"] if "auth/models" in t.get("file", "")]
...
def test_s84f_models_inserted_before_router():
    ...
    sdd = {"tasks": [
        ...
        {"id": "T2", "file": "src/auth/router.py", "agent": "backend", "title": "router"},
    ]}
    result = _ensure_auth_models_task(sdd)
    files = [t["file"] for t in result["tasks"]]
