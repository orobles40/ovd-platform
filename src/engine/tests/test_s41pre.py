...
class TestS41PATimeoutConstants:
    ...
    def test_research_timeout_definido(self):
        ...
        from graph import _RESEARCH_TIMEOUT, _ANALYZE_TIMEOUT
    ...
    def test_security_timeout_mayor_que_qa(self):
        ...
        from graph import _SECURITY_TIMEOUT, _QA_TIMEOUT
    ...
    def test_tests_timeout_es_io_puro(self):
        ...
        from graph import _TESTS_TIMEOUT, _QA_TIMEOUT
    ...
    def test_todos_configurables_por_env(self):
        ...
        for env_var in [
            ...
            assert env_var in src, f"{env_var} no encontrado como variable de entorno en graph.py"
...
class TestS41PAAnalyzeFr:
    ...
    async def test_analyze_fr_timeout_retorna_resultado_minimo(self):
        ...
        with patch("graph.invoke_structured", side_effect=_slow), \
            ...
            mock_tel.node_span.return_value.__aenter__ = AsyncMock(return_value=MagicMock())
        ...
        assert "Timeout" in result["fr_analysis"].get("summary", "") or \
               "Timeout" in result["fr_analysis"].get("raw", "")
...
class TestS41PBHeartbeat:
    ...
    def test_env_tiene_timeouts_diferenciados(self):
        ...
        if not env_path.exists():
            pytest.skip(".env no encontrado")
        content = env_path.read_text(encoding="utf-8")
        for var in ["OVD_AGENTS_TIMEOUT_SECS", "OVD_ANALYZE_TIMEOUT_SECS",
                    "OVD_SDD_TIMEOUT_SECS", "OVD_TESTS_TIMEOUT_SECS"]:
