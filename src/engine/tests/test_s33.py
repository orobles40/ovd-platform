...
class TestS33ANoModifyTests:
    ...
    def test_instruccion_es_prominente(self):
        ...
        feedback = result.get("retry_feedback", "")
        pos_instruccion = (
            feedback.find("INSTRUCCIÓN")
            if "INSTRUCCIÓN" in feedback
            else feedback.find("NUNCA")
        )
        pos_output = feedback.find("TESTS FALLIDOS")
        assert pos_instruccion != -1, "La instrucción no aparece en el feedback"
        assert pos_instruccion < pos_output, (
            "La instrucción debe aparecer antes que 'TESTS FALLIDOS' en el feedback"
        )
    ...
    def test_update_test_retry_feedback_contiene_instruccion(self):
        ...
        feedback = result.get("retry_feedback", "")
        assert (
            "NUNCA modifiques" in feedback
            or "no deben modificarse" in feedback
            or "INSTRUCCIÓN" in feedback
        )
...
"""
        ...
        assert len(lines) >= 3, (
            f"Debería detectar al menos 3 líneas relevantes, encontró: {lines}"
        )
