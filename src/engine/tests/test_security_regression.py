...
class TestLOW03RateLimiting:
    ...
    def test_login_endpoint_tiene_rate_limit(self):
        ...
        assert (
            hasattr(login, "_rate_limits")
            or hasattr(login, "__wrapped__")
            or any("limit" in str(getattr(login, attr, "")) for attr in dir(login))
...
class TestMEDIUM04HttpOnlyCookie:
    ...
    async def test_login_response_incluye_set_cookie(self, app):
        ...
        with (
            patch(
                "routers.auth_router._get_user_by_email",
                AsyncMock(return_value=user_record),
            ),
            patch(
                "routers.auth_router.issue_tokens", AsyncMock(return_value=fake_pair)
            ),
        ):
            async with AsyncClient(
                transport=ASGITransport(app=app), base_url="http://test"
    ...
    def test_logout_borra_cookie(self):
        ...
        assert (
            ...
            or "delete_cookie('ovd_refresh_token'" in source
