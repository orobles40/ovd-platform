...
def _make_conn_mock(
    user_row: dict | None = _USER_ROW, role: str = "admin", email: str = _EMAIL
):
    ...
    cursor_mock.fetchone = AsyncMock(
        side_effect=[
            tuple(user_row.values())
            if user_row
            else None,  # _get_user_by_email en login
    ...
    async def test_login_exitoso(self, app_with_mocks):
        ...
        with patch("psycopg.AsyncConnection.connect", return_value=conn):
            async with AsyncClient(
                ...
                r = await client.post(
                    "/auth/login", json={"email": _EMAIL, "password": _PWD_PLAIN}
                )
    ...
    async def test_login_usuario_no_existe(self, app_with_mocks):
        ...
        with patch("psycopg.AsyncConnection.connect", return_value=conn):
            async with AsyncClient(
                ...
                r = await client.post(
                    "/auth/login", json={"email": "noexiste@test.com", "password": "x"}
                )
...
class TestMe:
    ...
    async def test_me_token_invalido_retorna_401(self, app_with_mocks):
        async with AsyncClient(
            ...
            r = await client.get(
                "/auth/me", headers={"Authorization": "Bearer token.invalido.xxx"}
            )
        assert r.status_code == 401
...
class TestInjectCurrentUser:
    ...
    async def test_token_expirado_retorna_401(self, app_with_mocks, monkeypatch):
        ...
        async with AsyncClient(
            ...
            r = await client.get(
                "/auth/me", headers={"Authorization": f"Bearer {token_otro_secret}"}
            )
        assert r.status_code == 401
