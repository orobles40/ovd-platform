...
def _fr_fastapi():
    return {"raw": "API REST FastAPI con autenticación JWT. Endpoints de contratos.", "type": "feature"}


def _sdd_without_main_no_routers():
    return {
        "tasks": [
            {"id": "T1", "agent": "backend", "file": "src/auth/service.py", "title": "Auth service",
             "description": "lógica de autenticación", "depends_on": []},
            {"id": "T2", "agent": "backend", "file": "src/contracts/service.py", "title": "Contracts",
             "description": "lógica de contratos", "depends_on": []},
        ]
    }
...
def _sdd_without_main_with_routers():
    return {
        "tasks": [
            {"id": "T1", "agent": "backend", "file": "src/auth/router.py", "title": "Auth router",
             "description": "router de autenticación", "depends_on": []},
            {"id": "T2", "agent": "backend", "file": "src/contracts/router.py", "title": "Contracts router",
             "description": "router de contratos", "depends_on": []},
        ]
    }
...
class TestS70B:
    def test_no_router_hint_when_no_router_py_in_sdd(self):
        ...
        main_task = next((t for t in sdd["tasks"] if t.get("id") == "TASK-INFRA-MAIN"), None)
        assert main_task is not None
        desc = main_task["description"].lower()
        assert "include_router" not in desc, "No debe sugerir include_router cuando no hay router.py en SDD"
    ...
    def test_s70b_hint_mentions_no_routers(self):
        ...
        assert "no importes" in desc or "no hay router" in desc or "router que no estén" in desc
    ...
    def test_no_injection_when_main_already_present(self):
        ...
        sdd = {
            "tasks": [
                {"id": "T-MAIN", "agent": "backend", "file": "src/main.py", "title": "Main",
                 "description": "entry point", "depends_on": []},
            ]
        }
...
class TestS70C:
    def test_oracledb_mock_added_when_workspace_imports_it(self, tmp_path):
        ...
        (tmp_path / "tests" / "test_auth.py").write_text("def test_dummy(): pass\n", encoding="utf-8")
...
class TestS70D:
    def test_template_prohibits_self_import(self):
        ...
        assert "NUNCA importes desde el mismo módulo" in content or "auto-import" in content.lower()
