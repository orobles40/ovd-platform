...
class TestS81B:
    ...
    def test_s79c_actua_cuando_runner_es_none(self, tmp_path):
        ...
        self._write(tmp_path, "src/database.py",
...
class TestS81C:
    def test_identifica_tarea_de_tests_por_file(self):
        ...
        task = {"file": "tests/test_contracts.py", "title": "Tests", "description": "pytest"}
    ...
    def test_identifica_tarea_de_tests_por_titulo(self):
        ...
        from graph import _is_test_task_sdd
        task = {"file": "src/contracts/service.py", "title": "Escribir pytest para contratos", "description": ""}
        assert _is_test_task_sdd(task) is True

    def test_no_identifica_tarea_business_como_test(self):
        """S81-C: tarea business normal no es test task."""
        from graph import _is_test_task_sdd
        task = {"file": "src/contracts/service.py", "title": "Crear contrato", "description": "CRUD"}
    ...
    def test_tarea_test_no_eliminada_del_sdd(self):
        ...
        from graph import _is_test_task_sdd, _is_infra_task
        tasks = [
            {"file": "src/contracts/service.py", "title": "CRUD contratos", "description": ""},
            {"file": "src/contracts/router.py", "title": "Router contratos", "description": ""},
            {"file": "src/benefits/service.py", "title": "CRUD beneficios", "description": ""},
            {"file": "tests/test_contracts.py", "title": "Tests contratos", "description": "pytest"},
        ]
        cap = 2
        ...
        assert len([t for t in filtered if not _is_infra_task(t) and not _is_test_task_sdd(t)]) == cap
...
class TestS81D:
    def _fr(self) -> dict:
        return {"raw": "Sistema de contratos api rest con autenticación JWT login RUT", "type": "backend"}
    ...
    def test_inyecta_auth_router_con_path_exacto(self):
        ...
        main_task = next((t for t in result["tasks"] if t.get("file") == "src/main.py"), None)
    ...
    def test_no_inyecta_auth_router_con_auth_service_py(self):
        ...
        from graph import _ensure_fastapi_main_task
        sdd = {"tasks": [
            {"id": "T1", "agent": "backend", "file": "src/auth/service.py",
             "title": "Auth service", "description": "lógica auth"},
        ]}
        result = _ensure_fastapi_main_task(sdd, self._fr())
        main_task = next((t for t in result["tasks"] if t.get("file") == "src/main.py"), None)
        if main_task:
    ...
    def test_no_inyecta_auth_router_con_contracts_auth_router(self):
        ...
        from graph import _ensure_fastapi_main_task
        sdd = {"tasks": [
            {"id": "T1", "agent": "backend", "file": "src/contracts/auth_router.py",
             "title": "Contracts auth router", "description": ""},
        ]}
        result = _ensure_fastapi_main_task(sdd, self._fr())
        main_task = next((t for t in result["tasks"] if t.get("file") == "src/main.py"), None)
        if main_task:
