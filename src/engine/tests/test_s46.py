...
TEMPLATE = (
    pathlib.Path(__file__).parent.parent / "templates" / "system_frontend_react.md"
)
KNOWLEDGE_BASE = (
    ...
    def test_shadcn_componentes_listados(self):
        ...
        for comp in ["Card", "Table", "Dialog", "Badge", "Skeleton", "Alert"]:
            assert comp in content, (
                f"Componente shadcn/ui '{comp}' no mencionado en template"
            )
...
class TestS46BUIStates:
    ...
    def test_lista_estado_vacio(self):
        ...
        assert (
            "length === 0" in content
            or "Sin registros" in content
            or "estado vacío" in content.lower()
        )
...
class TestS46CKnowledgePatterns:
    ...
    def test_patterns_forms_validacion_inline(self):
        ...
        content = (KNOWLEDGE_BASE / "patterns-forms.md").read_text(encoding="utf-8")
        assert (
            "validateRut" in content
            or "validación inline" in content.lower()
    ...
    def test_patterns_data_table_sort_filtro_paginacion(self):
        ...
        content = (KNOWLEDGE_BASE / "patterns-data-table.md").read_text(
            encoding="utf-8"
        )
    ...
    def test_patterns_auth_forgot_password(self):
        ...
        content = (KNOWLEDGE_BASE / "patterns-auth.md").read_text(encoding="utf-8")
        assert (
            "ForgotPassword" in content
            or "forgot" in content.lower()
            or "recuperar" in content.lower()
        )
