...
def test_analyze_fr_invoca_publish_started():
    ...
    async def _run():
        ...
        with (
            patch.object(nats_client, "publish", side_effect=spy_publish),
...
def test_handle_escalation_invoca_publish_escalated():
    async def _run():
        ...
        with (
            ...
            patch(
                "graph.interrupt",
                return_value={"resolution": "Revisado por arquitecto"},
            ),
        ):
...
def test_orden_de_eventos_en_ciclo_normal():
    ...
    async def _run():
        ...
        fr_output = g.FRAnalysisOutput(
            ...
            oracle_involved=False,
            risks=[],
            summary="Dashboard para reporting",
        )
        mock_chain = AsyncMock()
        mock_chain.ainvoke = AsyncMock(return_value=fr_output)
        mock_llm = MagicMock()
    ...
    started_idx = next(
        (i for i, s in enumerate(subjects) if "session.started" in s), -1
    )
    approved_idx = next(
        (i for i, s in enumerate(subjects) if "session.approved" in s), -1
    )
